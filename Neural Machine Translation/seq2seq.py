...
class EncoderRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, dropout_p=0.1):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.lstm_cell_left = LSTMCell(hidden_size, hidden_size)
        self.lstm_cell_right = LSTMCell(hidden_size, hidden_size)
    ...
    def forward(self, input):
        ...
        hc_left = self.get_initial_hidden_state()
        hc_right = self.get_initial_hidden_state()
        outputs_left = torch.zeros(input_length, self.hidden_size, device=device)
        outputs_right = torch.zeros(input_length, self.hidden_size, device=device)
        for i in range(input_length):
            i_r = input_length - i - 1
            hc_left = self.lstm_cell_left(self.dropout(self.emb(input[i])), hc_left)
            hc_right = self.lstm_cell_right(self.dropout(self.emb(input[i_r])), hc_right)
            outputs_left[i] += hc_left[0][0]
            outputs_right[i_r] += hc_right[0][0]
        output = torch.cat((outputs_left, outputs_right), dim=1)
        h_left, c_left = hc_left
        h_right, c_right = hc_right
        h = torch.tanh(self.fh(torch.cat((h_left, h_right), dim=1)))
        c = torch.tanh(self.fc(torch.cat((c_left, c_right), dim=1)))
        return output, (h,c)
...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        self.lstm_cell = LSTMCell(hidden_size*3, hidden_size)
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        weighted_encoder = torch.bmm(attn_weights.unsqueeze(1), encoder_outputs).squeeze(1)
        lstm_input = torch.cat([weighted_encoder, input_enc], dim=1)
        h, c = self.lstm_cell(lstm_input, hidden)
        hidden = (h, c)
        #print("h:",h.shape, " input_enc:", input_enc.shape," weighted_encoder:",weighted_encoder.shape)
        out_tensor = self.out(torch.cat((h, weighted_encoder, input_enc), dim = 1))
        log_softmax = out_tensor.log_softmax(1)
...
def train(input_tensor, target_tensor, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    ...
    optimizer.zero_grad()
    hc = encoder_hidden
    encoder_states, hc = encoder(input_tensor)
    decoder_log_softmax = []
    for y in [torch.tensor([SOS_index])] + list(target_tensor.unbind(0))[:-1]:
        log_softmax, hc, attn_weights = decoder(y, hc, encoder_states.unsqueeze(0))
        decoder_log_softmax.append(log_softmax)
    decoder_log_softmax = torch.stack(decoder_log_softmax).permute(1, 0, 2)
    loss = criterion(decoder_log_softmax.flatten(0, 1), target_tensor.T.flatten())
    loss.backward()
    optimizer.step()
    optimizer.zero_grad()
